...
<div class="profil-container">
    <a href="gestionnaire.php?action=dashboard" class="btn-back">
    ...
    <div class="profil-card">
        <div class="profil-header">
            <div class="profil-avatar">
                <?= strtoupper(substr($gestionnaire['nom'], 0, 1)) ?>
            </div>
            <h2>Mon profil</h2>
        ...
        <div class="profil-body">
            ...
            <form id="profilForm">
                <div class="form-group">
                    ...
                    <input type="text" name="nom" id="nom" value="<?= htmlspecialchars($gestionnaire['nom']) ?>" required>
                </div>
                
                <div class="form-group">
                    <label><i class="fa-solid fa-phone"></i> Téléphone</label>
                    <input type="tel" name="telephone" id="telephone" value="<?= htmlspecialchars($gestionnaire['telephone']) ?>" required>
                </div>
                
                <div class="form-group">
                    <label><i class="fa-solid fa-envelope"></i> Email</label>
                    <input type="email" name="email" id="email" value="<?= htmlspecialchars($gestionnaire['email']) ?>" required>
                </div>
...
document.getElementById('profilForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('gestionnaire.php?action=mettre_a_jour_profil', {
            method: 'POST',
            body: formData
        });
        ...
        if (data.success) {
            ...
            setTimeout(() => {
                window.location.href = 'gestionnaire.php?action=dashboard';
            }, 1500);
        } else {
            let errors = '';
