...
<head>
    ...
    <style>
        ...
        .sidebar { width: 280px; background: linear-gradient(180deg, #0a2b5e 0%, #0f2c3f 100%); color: white; display: flex; flex-direction: column; position: fixed; height: 100vh; overflow-y: auto; transition: transform 0.3s ease; z-index: 200; }
        ...
        @media (max-width: 900px) {
            ...
            .modal-choix-content, .modal-consultation-content, .modal-qr-content { max-width: 100%; margin: 0; border-radius: 16px 16px 0 0; }
            .modal-choix, .modal-consultation, .modal-qr { align-items: flex-end; }
        }
                ...
                <div id="consultationsContent">
                    ...
                    <div style="overflow-x:auto;">
                        ...
                        <tr>
                            ...
                            <td><?= $c['heure_passage_estimee'] ? date('H:i', strtotime($c['heure_passage_estimee'])) : '—' ?></td>
                            <td><?php $bc = match($c['statut']) { 'traite' => 'badge-green', 'en_cours' => 'badge-blue', 'annule','absent' => 'badge-red', default => 'badge-grey' }; $lb = match($c['statut']) { 'traite' => 'Traitée', 'en_cours' => 'En cours', 'annule' => 'Annulée', 'absent' => 'Absent', 'en_attente' => 'En attente', 'confirme' => 'Confirmé', default => $c['statut'] }; ?><span class="badge <?= $bc ?>"><?= $lb ?></span></td>
                        </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
...
<script>
    ...
    function afficherFile(file, filtered) {
        ...
        for(const f of page){
            ...
            h+=`<tr ${rowStyleG}>
                <td><div class="rang-badge ${f.statut==='en_cours'?'actif':''}">${f.rang}</div></td>
                <td><strong>${escapeHtml(f.patient_nom)} ${escapeHtml(f.patient_prenom)}</strong>${pauseInfo}</td>
                <td>${escapeHtml(f.telephone)}</td>
                <td>${f.heure_passage_estimee||'—'}</td>
                <td>${f.mode_prise==='LIGNE'?'<span class="badge badge-blue">En ligne</span>':'<span class="badge badge-green">Sur place</span>'}</td>
    ...
    function afficherTableauHistoriqueG(rows) {
        ...
        for(const v of rows) {
            h += `<tr>
                ...
                <td><span class="badge ${bcMap[v.statut]||'badge-grey'}">${lbMap[v.statut]||v.statut}</span></td>
    ...
    function filtrerFile(q){ const t=q.toLowerCase().trim(); if(!t){ afficherFile(_fileData); return; } const f=_fileData.filter(p=>(p.patient_nom||'').toLowerCase().includes(t)||(p.patient_prenom||'').toLowerCase().includes(t)||(p.patient_telephone||'').toLowerCase().includes(t)); afficherFile(f, true); }
