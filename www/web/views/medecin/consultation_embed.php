...
<div class="emb-modal-bg" id="embModalPause">
    <div class="emb-modal">
        ...
        <div class="emb-modal-btns">
            ...
            <button class="emb-btn emb-btn-pause" onclick="embConfirmerPause()"><i class="fas fa-pause"></i> Confirmer</button>
        </div>
    </div>
</div>
...
<div class="emb-modal-bg" id="embModalRdv">
    <div class="emb-modal">
        ...
        <div class="emb-modal-btns">
            <button class="emb-btn emb-btn-refresh" onclick="document.getElementById('embModalRdv').classList.remove('open')">Annuler</button>
            <button class="emb-btn emb-btn-start" onclick="embConfirmerRdv()"><i class="fas fa-calendar-check"></i> Fixer le RDV</button>
        </div>
    </div>
</div>
...
/* ══ AJAX actions ══ */
function embPost(action, body, successMsg) {
    ...
    .then(r => r.json())
    .then(d => {
        if (d.success) { embMsg(successMsg || 'Succès', 'success'); embRafraichir(); }
        else embMsg(d.message || 'Une erreur est survenue.', 'error');
    })
    .catch(() => embMsg('Erreur réseau. Veuillez réessayer.', 'error'));
}
...
function embFermerPause() { document.getElementById('embModalPause').classList.remove('open'); _embPauseId = null; }
function embConfirmerPause() {
    const motif = document.getElementById('embPauseMotif')?.value.trim() || 'Examen externe';
    embFermerPause();
    embPost('mettre_en_pause_ajax', `consultation_id=${_embPauseId}&motif=${encodeURIComponent(motif)}`, 'Consultation mise en pause.');
}
...
function embConfirmerRdv() {
    ...
    if (!date) { embMsg('Veuillez choisir une date.', 'error'); return; }
    document.getElementById('embModalRdv').classList.remove('open');
    embPost('fixer_prochain_rdv_ajax', `consultation_id=${_embRdvId}&date_rdv=${date}&notes=${encodeURIComponent(notes)}`, 'Prochain RDV fixé.');
}
